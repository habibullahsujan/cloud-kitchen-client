...
import { getUserInfo, updateLikedStatus } from "../../Apis/user";
...
const Details = () => {
  ...
  const handleBuyNow = () => {
    const ordersInfo = {
      ...
      productPrice: serviceDetails?.price,
      productImg: serviceDetails?.picture,
      productNo: serviceDetails?.serviceNo,
  ...
  const handleClickLike = () => {
    ...
    updateLikedStatus(user?.email, productInfo)
    ...
    <div>
      <div className="flex justify-center my-16">
        <div className="flex flex-col max-w-lg p-6 space-y-6 overflow-hidden rounded-lg shadow-md bg-yellow-100 text-black">
              ...
              {user ? (
                ...
                >
                  <svg
                    ...
                    className={`w-6 h-8 fill-current dark:text-violet-400 ${
                      commentActive && "text-orange-400"
                    }`}
                  >
              ...
              {user ? (
                ...
                >
                  <svg
                    ...
                    className={`w-6 h-8 fill-current dark:text-violet-400 ${
                      likeActive && loggedUser?.liked?.isLiked && "text-orange-400"
                    }`}
                  >
          ...
          <Link onClick={handleBuyNow} to={`/payment/${serviceDetails._id}`}>
            {" "}
            <button className="py-3 bg-blue-400 text-white font-bold w-full">
              Buy Now
            </button>{" "}
          </Link>
        </div>
